...
const { getChordKeys, getChordKeysFromRootKey } = require('../chords/chord_utils');
...
function getScaleChordKeysFromRootKey(root_key, scale_type) {
    ...
    return keys.map((key, i) => {
        ...
        if (isMajor(roman_numeral)) {
            return getChordKeysFromRootKey(key, 'major');
        } else if (isMinor(roman_numeral)) {
            return getChordKeysFromRootKey(key, 'minor');
        } else if (isAugmented(roman_numeral)) {
            return getChordKeysFromRootKey(key, 'augmented');
        } else if (isDiminished(roman_numeral)) {
            return getChordKeysFromRootKey(key, 'diminished');
        }
    })
}
