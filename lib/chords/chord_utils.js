...
function getChordNotesFromRootKey(root_key, chord_type, scale_root_note, scale_type) {
    ...
    const chord_keys = getChordKeys(root_key, chord_type);
...
function getChordTypeFromRomanNumeral(roman_numeral) {
    if (isMajor(roman_numeral)) {
        ...
        return 'augmented';
    } else if (isDeminished(roman_numeral)) {
        return 'diminished';
    } else {
...
module.exports = { getChordKeys, getChordNotesFromRootKey, getChordNotesFromRootNote, isAugmented, isDeminished, isMajor, isMinor }